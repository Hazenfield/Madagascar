...
**See what the register holds about you.** Your own records, and those you are
...
person it belongs to, on their own device, instead of from a stack of
...
A transfer of a parcel is done as a single event, in person:
...
Why this is the right shape: consent is the hardest thing to prove about a land
sale, and every attempt to prove it on paper is an attempt to reconstruct after
...
- **Parties without a smartphone.** Common, especially for rural sellers. There
  has to be a fallback, and it must not quietly reopen the hole the ceremony
  closes.
- **Presence is not consent.** Attending proves attendance, not free will.
  Coercion looks identical to agreement from the outside.
- **A phone proves a device, not a person.** The real trust anchor is
  enrolment — how a person is bound to a device in the first place. That is where
  the fraud will move to.
- **Succession does not fit this shape at all.** When a transfer follows a death,
  the previous holder cannot attend. Inheritance needs its own procedure, and the
  ceremony covers sale only.
